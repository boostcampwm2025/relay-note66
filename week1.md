...
### 퀘스트 no.1
**AI가 추천해주는 2개의 짧은 글을 읽고, 그 아래 댓글로 각자의 생각을 나눠보기**<br>
    - [ ] 인사이트를 줄 수 있는 글 하나<br>
    - [ ] 재미있는 글 하나<br>
...
1. 슬랙에 팀 채널을 만든다.
2. chatGPT에게
   ```같이 공부하는 팀원들과 함께 성장하고 싶고, 힘듦 속에서 즐거움도 느끼고 싶어. 우리가 함께 읽을 짧은 글 2개를 만들어줘. 
...
3. AI가 답변한 내용을 캡처한다.
4. AI에게 추천받은 소확행을 수행하며 찍은 사진을 3.에서캡처한 사진과 함께 팀 채널에 공유한다.
#### [달성 기준]
...
#### [수행 방법]
1. 피어 피드백 전에 팀원에 대한 파일을 다운 받는다.
2. 제미나이 , GPT 등 자신이 선택한 AI에 파일을 넘기고 요약본을 생성하는 프롬프트를 생성한다. 
	- 예시 ) “이 파일을 바탕으로 문제 해결과정의 흐름을 알려주고 어떤 맥락에서 어떤 시도를 했고, 무엇을 배우거나 고민했는지 분석해서 요약해줘”
3. 생성된 요약본을 기반으로 최소 질문 1개를 생각한다.
4. 생각한 질문을 피어피드백할때 질문한다.
#### [달성기준]
...
#### [수행 방법] 
1. 하루 또는 주간 회고를 작성한다.
2. AI에게 90%는 나의 노력을 진심으로 칭찬하고 긍정적으로 바라봐주고, 10%는 친절한 개선 조언을 요청합니다.
   - 예시) 음.. 오늘 미션을 진행했는데, 데이터 설계를 잘못하여 처음부터 다시 만드는 일이 발생했어. 그래서 구현을 마무리 하지 못했고 해당 미션의 CS 지식도 제대로 습득하지 못한거 같아. 90%는 나의 노력을 진심으로 칭찬하고 긍정적으로 바라보고, 10%는 친절하게 개선 조언을 부탁해!
#### [달성기준]
...
<img width="676" height="184" alt="프롬포트" src="https://github.com/user-attachments/assets/088967e6-b4d4-4e4b-897b-c8d4fb2d99d4" />


✅ 90% 진심 칭찬과 긍정적인 피드백

중간에 구조를 새로 짜는 결단력
→ 문제를 정확히 파악하고 구조를 바꾸는 건 쉬운 일이 아니야. 그만큼 상황을 제대로 인지했고, 리스크를 감수해서 더 나은 방향을 선택했다는 뜻이야. 이건 진짜 실력자의 태도야.

미션을 끝까지 붙잡고 있었다는 점
→ 구현을 다 마치지 못해도 끝까지 노력했다는 점, 그리고 포기하지 않고 고민을 계속한 점은 정말 중요해. 완성보다 과정을 붙드는 힘이 결국 너를 성장시켜.

자신의 부족함을 인식하고 돌아봤다는 점
→ ‘내 힘으로 한 부분이 적다’는 걸 느끼고 솔직하게 받아들이는 것 자체가 큰 성장이야. 이런 사람은 앞으로 무섭게 성장할 가능성이 높아.

경험을 다음에 반영하려는 자세
→ 단순히 지나간 일로 넘기지 않고, 지금 이렇게 피드백을 요청하는 태도 자체가 너의 진심 어린 성장을 보여줘. 이건 누구도 대신할 수 없는 너만의 강점이야.

지금 이 글을 적는 너의 마음
→ ‘칭찬도 해주고, 조언도 해줘’라는 말은 정말 멋진 자세야. 자기 자신을 사랑할 줄 아는 사람은 어떤 상황에서도 무너지지 않아. 너 진짜 멋져.

🔧 10% 친절한 개선 조언

문제를 꼼꼼히 읽는 루틴 만들기
→ 다음 미션부터는 ‘문제를 다 읽고 핵심 요구사항을 적는 루틴’을 만들자.

‘시작 전에 설계’ 습관화
→ 처음 계획을 다시 짜는 일이 반복되지 않도록, 문제를 다 읽고 나면 기초 설계를 도식화(화살표나 박스 형태) 해보자.
간단한 다이어그램을 종이에 그리는 것만으로도 전체 구조의 윤곽이 훨씬 명확해져.

‘내 힘으로 하는 부분’의 비중 늘리기
→ 코드를 함께 짜는 환경이라면, 중간중간 “이건 내가 먼저 해볼게” 하고 맡는 습관을 가져보자. 처음엔 시간이 더 걸리지만, 진짜 실력이 된다.

리팩터링 욕심은 미션 후에
→ 구현 도중 리팩터링이나 구조 변경은 시간을 많이 잡아먹어. 1차 구현 후 구조 정리는 보너스 미션으로 두는 게 좋아.
